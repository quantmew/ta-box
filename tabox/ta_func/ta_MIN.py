...
def TA_MIN_Lookback(optInTimePeriod: cython.int) -> cython.Py_ssize_t:
...
def TA_MIN(
    ...
    if optInTimePeriod < 100:
        return TA_MIN_SMALL_TIMEPERIOD(
            startIdx, endIdx, inReal, optInTimePeriod, outReal
        )
    else:
        return TA_MIN_LARGE_TIMEPERIOD(
            startIdx, endIdx, inReal, optInTimePeriod, outReal
        )
