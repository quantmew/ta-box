...
def TA_ADOSC(
    ...
    optInFastPeriod: cython.Py_ssize_t,
    optInSlowPeriod: cython.Py_ssize_t,
    ...
    slowest_period: cython.Py_ssize_t = 0
    if optInFastPeriod < optInSlowPeriod:
        slowest_period = optInSlowPeriod
    else:
        slowest_period = optInFastPeriod
    ...
    # Calculate lookback
    lookback: cython.Py_ssize_t = TA_EMA_Lookback(slowest_period)
    ...
    today: cython.Py_ssize_t = startIdx - lookback

    # Calculate EMA k value
    fastk: cython.double = 2.0 / (optInFastPeriod + 1)
    one_minus_fastk: cython.double = 1.0 - fastk

    slowk: cython.double = 2.0 / (optInSlowPeriod + 1)
    one_minus_slowk: cython.double = 1.0 - slowk

    # Initialize AD and EMA
    ad: cython.double = 0.0
    high: cython.double = inHigh[today]
    low: cython.double = inLow[today]
    close: cython.double = inClose[today]
    tmp: cython.double = high - low
    if tmp > 0.0:
        ad += (((close - low) - (high - close)) / tmp) * inVolume[today]
    today += 1

    fastEMA: cython.double = ad
    slowEMA: cython.double = ad
