...
def TA_ROCR_Lookback(optInTimePeriod: cython.int) -> cython.Py_ssize_t:
    """
    TA_ROCR_Lookback - Rate of change ratio Lookback
...
def ROCR(real: np.ndarray, timeperiod: int = 10) -> np.ndarray:
    ...
    Rate of change ratio (Overlap Studies)

    The ROCR is calculated as (price / prevPrice), where prevPrice is the price
    from 'timeperiod' periods ago.

    Inputs:
        real: (any ndarray) Input series
    Parameters:
        timeperiod: 10 Number of periods for the calculation
    Outputs:
        real
    """
    ...
    lookback: cython.Py_ssize_t = startIdx + TA_ROCR_Lookback(timeperiod)
